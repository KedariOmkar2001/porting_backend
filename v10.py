...
def generate_queries(designations_df, offices_df, employees_df, config: GenerationConfig, user_role_df):
    ...
    for index, row in employees_df.iterrows():
        try:
            ...
            # MODIFIED: Now we need TWO user_role_ids per user
            user_role_id_1 = config.starting_user_role_id + (success_count * 2)  # First role
            user_role_id_2 = config.starting_user_role_id + (success_count * 2) + 1  # Second role

            user_role_id_from_role_name = get_role_id_of_user_from_umsm_role_table(designation, user_role_df)
            default_role_if_for_all_user = get_role_id_of_user_from_umsm_role_table("DEFAULT USER",user_role_df)
            ...
            excel_preview.append(
                f"--   Date of Joining       : {date_of_joining if pd.notna(date_of_joining) else 'N/A'}")
            excel_preview.append(
            ...
            excel_preview.append(
                f"--   Contact Number        : {sql_val(contact_no, is_int=True) if contact_no else 'N/A'}")
            excel_preview.append(
                f"--   Company/Organization  : {company_organization if company_organization else 'N/A'}")
            excel_preview.append("")
            ...
            excel_preview.append(
                f"--   Role ID ({designation if designation else 'N/A'})     : {user_role_id_from_role_name if user_role_id_from_role_name else 'NOT FOUND'}")
            excel_preview.append(f"--   Role ID (Default User)       : {default_role_if_for_all_user if default_role_if_for_all_user else 'NOT FOUND'}")
            excel_preview.append("")
            ...
            excel_preview.append(f"--   User Role ID 1        : {user_role_id_1}")
            excel_preview.append(f"--   User Role ID 2        : {user_role_id_2}")
            excel_preview.append("")
            excel_preview.append("-- SECURITY:")
            excel_preview.append(f"--   Email Encrypted       : {encrypted_email if encrypted_email else 'NO'}")
            excel_preview.append(f"--   Phone Encrypted       : {encrypted_phone if encrypted_phone else 'NO'}")
            excel_preview.append(f"--   Password (Hashed)     : {hashed_password}")
            excel_preview.append("")
            excel_preview.append("--" * 80)
            excel_preview.append("")
    ...
    true
            ...
            # Query 4a: um.umst_user_role (First role - Dynamic from designation)
            user_role_query_1 = f"""INSERT INTO um.umst_user_role (
    ...
    true
            ...
            # Query 4b: um.umst_user_role (Second role - Fixed role_id 2065456361)
            user_role_query_2 = f"""INSERT INTO um.umst_user_role (
    user_role_id, uid, role_id, unit_id, operated_on, operated_by, tenant_id, is_data_ported
) VALUES (
    {user_role_id_2}, 
    {uid}, 
    ...
    true
            ...
            um_queries.append("-- [1/3] um.umsm_user")
            um_queries.append(user_query)
            um_queries.append("")
            um_queries.append("-- [2/3] um.umst_user_role (Dynamic Role from Designation)")
            um_queries.append(user_role_query_1)
            um_queries.append("")
            um_queries.append("-- [3/3] um.umst_user_role (Fixed Role ID: 2065456361)")
            um_queries.append(user_role_query_2)
            um_queries.append("")
